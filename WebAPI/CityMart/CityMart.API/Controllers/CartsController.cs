    ...
    public class CartsController : ControllerBase
    {
        ...
        public async Task<ActionResult<CartDto>> AddToCart([FromBody] AddToCartRequest request)
        {
            try
            {
                ...
                return Ok(new { success = true, message = "Product added to cart", data = cart });
            }
            ...
            catch (Exception ex)
            {
                ...
                return StatusCode(500, new { success = false, message = "Error adding product to cart" });
            }
        }
        ...
        public async Task<ActionResult<CartDto>> UpdateCartItem(int cartItemId, [FromBody] UpdateCartItemRequest request)
        {
            try
            {
                ...
                return Ok(new { success = true, message = "Cart item updated", data = cart });
            }
            catch (ArgumentException ex)
            {
                return BadRequest(new { success = false, message = ex.Message });
        ...
        [HttpGet("total")]
        [Authorize]
        public async Task<ActionResult> GetCartTotal()
        {
            try
            {
                var userId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
                ...
                return Ok(new { success = true, data = new { totalPrice = total } });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error getting cart total");
                return StatusCode(500, new { success = false, message = "Error getting cart total" });
            }
        }
    }
