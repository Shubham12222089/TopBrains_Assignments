...
namespace CityMart.API.Services
{
    public interface IProductService
    {
        Task<PaginatedResult<ProductDto>> GetProductsAsync(PaginationParams paginationParams);
    ...
    public class ProductService : IProductService
    {
        ...
        public async Task<PaginatedResult<ProductDto>> GetProductsAsync(PaginationParams paginationParams)
        {
            ...
            {
                query = query.Where(p => p.CategoryId == paginationParams.CategoryId);
            }

            if (paginationParams.MinPrice.HasValue)
            {
                query = query.Where(p => p.Price >= paginationParams.MinPrice);
            }

            if (paginationParams.MaxPrice.HasValue)
            {
                query = query.Where(p => p.Price <= paginationParams.MaxPrice);
            }
            ...
            query = paginationParams.SortBy.ToLower() switch
            {
                ...
                _ => paginationParams.SortDescending ?
            ...
            return new PaginatedResult<ProductDto>(items, totalCount, paginationParams.PageNumber, paginationParams.PageSize);
        }
        ...
        public async Task<ProductDto> CreateProductAsync(CreateProductDto createProductDto)
        {
            if (string.IsNullOrWhiteSpace(createProductDto.Name))
                throw new ArgumentNullException(nameof(createProductDto.Name));
        ...
        public async Task<ProductDto> UpdateProductAsync(int id, UpdateProductDto updateProductDto)
        {
            ...
            if (updateProductDto.Stock.HasValue && updateProductDto.Stock >= 0)
